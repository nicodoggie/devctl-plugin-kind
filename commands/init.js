...
module.exports = {
  ...
  run: async (toolbox) => {
    ...
    const defaultClusterName = basename(process.cwd());
    ...
    const mergedConfig = {
      ...defaults,
      ...(willCustomize ? customizeKind(toolbox) : {}),
    };
    ...
    const nodes = mergedConfig.nodes.map((node) => {
      ...
      if (patches) {
        const kubeadmPatches = yaml.safeDump(patches);
