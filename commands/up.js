...
module.exports = {
  ...
  run: async function ({ print, system, kindConfig, parameters }) {
    ...
    print.info(
      `\nRunning bootstrap scripts for kind cluster ${clusterName}...`
    );
    ...
    print.info(`\nBootstrapped kind cluster '${clusterName}'!`);
    ...
    let pvExists = false;
    try {
      print.info("\nChecking for repository mount PersistentVolume...");
    ...
    let pvcExists = false;
    try {
      print.info("\nChecking for repository mount PersistentVolumeClaim...");
    ...
    print.info(
      `You can now use the ${clusterName} cluster! Run devctl kind:switch to deploy your development pods!`
    );
  },
};
